import { displayRoadmapTitle } from './playground-roadmap-data'
...
const rawHomepageRoadmap: Record<HomeLocale, HomepageRoadmapChapter[]> = {
  zh: [
    {
      id: 'intro-map',
      title: 'AI × Web3 入门地图',
...
function localizeHomepageRoadmap(chapters: HomepageRoadmapChapter[], locale: HomeLocale): HomepageRoadmapChapter[] {
  if (locale === 'en') return chapters
  return chapters.map((chapter) => ({
    ...chapter,
    groups: chapter.groups.map((group) => ({
      ...group,
      title: displayRoadmapTitle(group.title, locale),
      topics: group.topics.map((topic) => ({
        ...topic,
        title: displayRoadmapTitle(topic.title, locale),
      })),
    })),
  }))
}
...
export const homepageRoadmap: Record<HomeLocale, HomepageRoadmapChapter[]> = {
  zh: localizeHomepageRoadmap(rawHomepageRoadmap.zh, 'zh'),
  en: rawHomepageRoadmap.en,
}
