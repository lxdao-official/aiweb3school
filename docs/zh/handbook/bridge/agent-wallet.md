...
**难度：中级。** 先理解 Account Abstraction 如何让账户表达规则，而不只是由一把私钥控制。
...
**难度：中级。** Smart Account 是 Agent Wallet 的执行边界，用合约规则承载权限、恢复和自动化。
...
**难度：中级。** Safe 适合理解团队、DAO 和高价值账户如何用多签和模块分散执行权。
...
**难度：高级。** Session Key 是让 Agent 自动执行低风险动作的关键，但必须限制时间、额度、目标和方法。
...
**难度：高级。** Policy 把“Agent 可以做什么”写成系统能检查的规则。
...
**难度：高级。** Guard 是确定性拦截层，负责拒绝不符合 policy 的交易或工具调用。
...
**难度：中级。** Simulation 用来在签名和广播前预演交易结果，降低错误执行的概率。
...
**难度：中级。** Revocation 关注权限如何被用户或系统及时收回。
...
**难度：初级。** Human Check 不是所有步骤都手动确认，而是在关键风险点让用户看懂并决定。
