...
- When changing static serving or deployment behavior, verify pretty URLs such as `/zh/` resolve to their directory `index.html`, and missing static assets under paths like `/assets/*.css` or `/assets/*.js` return 404 instead of falling back to HTML.
